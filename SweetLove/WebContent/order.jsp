...
<div id="contentCon">
	<div class="left">
        ...
        <ul>
            ...
            <li><a href="myorder?uid=${user.data.id}" class="my">我的订单</a></li>
    ...
    <div class="right">
    	<ul>
        	<li id="allorder" onclick="orderall()" class="all">全部订单</li>
            <li id="waitpay" style="margin-left: 150px">待付款</li>
            <li id="yetsend" style="margin-left: 150px">已发货</li>
            <li id="waitevaluate" style="margin-left: 150px">已收货</li>
<!--             <li>售后服务</li> -->
        </ul>
        <!-- 页面接在，设置表头样式 -->
        <script type="text/javascript">
        	...
        	if(code==0){//全部订单
        		//改变样式
        		$("#allorder").removeClass("all");
        		$("#waitpay").removeClass("all");
        		$("#yetsend").removeClass("all");
        		$("#waitevaluate").removeClass("all");
        		$("#allorder").addClass("all");
        	}
        	if(code==1){//待付款订单
        		//改变样式
        		$("#allorder").removeClass("all");
        ...
        <script type="text/javascript">
        	$("#waitpay").click(function(){
        		...
        		var uid=$("#id").val();
        		//向后台发送请求
        		location.href="waitpayOrder?uid="+uid;
            	/* $.ajax({
            		url:"/SweetLove/waitpayOrder",//请求路径，需要写完整路径名
        			type:"get",//请求方式
        			data:{"uid":uid},//请求参数
        ...
        <!-- 点击已发货按钮 -->
        <script type="text/javascript">
        	$("#yetsend").click(function(){
        		//获取用户id
        		var uid=$("#id").val();
        		location.href="yetOrder?uid="+uid;
        	})
        </script>
        <!-- 点击待评价订单 -->
        <script type="text/javascript">
        	$("#waitevaluate").click(function(){
        		...
        		var uid=$("#id").val();
        		//向后台发送请求
        		location.href="waitevaluate?uid="+uid;
        	})
        </script>
        
        <c:forEach var="orderItem" items="${orderItemAll.data}" varStatus="s">
	        <ol>
	        	<input type="checkbox">
	        	<!-- 订单id -->
	        	<input type="hidden" value="${orderItem.orderid}">
	            <p>${orderItem.ordertime}&nbsp;&nbsp;&nbsp;&nbsp;订单号：${orderItem.ordernumber}</p>
	            <a href="javascript:void(0)" onclick="del(this)"></a><!-- 删除按钮 -->
	        </ol>
	        <div>
	        	<li style="background: url(/goodsphoto/${orderItem.sample1}) center no-repeat;"></li>
	            <ul>
	            	<li>
	                	<p>${orderItem.goodsname}</p>
	                    <span>规格：${orderItem.spec}</span>
	                </li>
	                <li class="price">¥${orderItem.price}</li>
	                <li class="price">x${orderItem.goodsnumber}</li>
	                <li>
	                	<a href="javascript:void(0)">联系我们</a>
	                    <a href="javascript:void(0)" class="text02">申请售后</a>
	                </li>
	                <li>
	                	<a href="javascript:void(0)">订单详情</a>
	                    <a href="javascript:void(0)" class="text02">查看物流</a>
	                </li>
	            </ul>
	        </div>
        </c:forEach>
        <script type="text/javascript">
        	...
        	function del(t){
        		...
        		//console.info(uid);
        		//获取需要删除的订单id
        		var orderid=$(t).prev().prev().val();
        		//console.info(orderid);
        		//向后台请求
        		location.href="delOrder?uid="+uid+"&orderid="+orderid;
        	}
        </script>
